...
:image: @img/ds1302-arduino/thumbnail.jpg
:tags: arduino rtc
:experimental:
:stem:

Date/time and RAM examples are listed in the xref:ds130x-reference.adoc[DS130X Common Reference]. This page covers DS1302-specific features.
...
* Can trickle-charge its battery by outputting voltage from its V~CC1~ pin.

|===
|Pin |Pin name |Function

|1 |V~CC2~ |5V power supply
|2 |X1 .2+|Connection to 32.768 kHz quartz crystal
|3 |X2
|4 |GND |Ground
|5 |CE |Chip enable
|6 |I/O |Serial data
...
image::ds1302-arduino/schematic.png[DS1302 schematic]
...
This project uses my *DS130X* library. link:/zipped/DS130X.zip[Download the library in Zip format]
...
The example code is at menu:File[Examples > DS130X > DS1302 > Other > SetTCR]. It does not output anything to the serial monitor.
...
[source,cpp]
----
include::../res/downloads/DS130X/examples/DS1302/Other/SetTCR/SetTCR.ino[]
----
...
* stem:[I_(max)]: Output current (A)
* stem:[D]: Diode drop, e.g., voltage drop across a diode (typically 0.7 V); multiply this by the number of diodes you selected
* stem:[R_1]: Resistor value you selected (&#8486;)

For example, the output current of having 1 diode and the 2k resistor selected would be:

[stem]
++++
I_(max) = (5-0.7)/(2000) = 0.00215 "A" ~~ 2.2 "mA"
++++
...
* `TCR_1D_2R`: 1 diode and 2 k&#8486; resistance
* `TCR_1D_4R`: 1 diode and 4 k&#8486; resistance
* `TCR_1D_8R`: 1 diode and 8 k&#8486; resistance
* `TCR_2D_2R`: 2 diodes and 2 k&#8486; resistance
* `TCR_2D_4R`: 2 diodes and 4 k&#8486; resistance
* `TCR_2D_8R`: 2 diodes and 8 k&#8486; resistance
...
NOTE: To get the current TCR setting, read from the `TCR_GET` register. `readRaw` is preferred since it returns the raw BCD data, like how `writeRaw` writes raw data.
